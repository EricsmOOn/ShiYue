...
%%%-------------------------------------------------------------------
-module(log_resource_client).
-author("ericwong").

-behaviour(gen_server).

-import(log_resource_server, [request_resource/2, return_resource/1]).

%% API
-export([start_link/0, start/0,stop/1, resource_already_release/2]).
...
init([]) ->
  ...
  io:format("[~s] - Cilent[~20w]:online~n", [get_now_time(),Name]),
  erlang:send_after(rand:uniform(500), self(), {request_resource_server, Name}),
  {ok, -1}.
...
-spec(handle_info(Info :: timeout() | term(), State :: #state{}) ->
  ...
  {noreply, NewState :: #state{}, timeout() | hibernate} |
  {stop, Reason :: term(), NewState :: #state{}}).
handle_info({request_resource_server, Name}, State) ->
  Amount = random_num(1, ?MAX_REQUEST_AMOUNT),
  Time = random_num(0, ?MAX_HOLD_TIME),
  Return_Time = random_num(0, ?INTERVAL),
  io:format("[~s] - Cilent[~20w]:{Amount,~p},{Time,~p}~n", [get_now_time(),Name, Amount, Time]),
  NewState =
    try request_resource(Amount, Time) of
      ...
      _ -> State
        ...
        _ : _ -> exit(server_exit)
    end,
  erlang:send_after(?INTERVAL, self(), {request_resource_server, Name}),
  {noreply, NewState};

handle_info({return_resource, Name}, State) ->
  io:format("[~s] - Cilent[~20w]:{~p},{Key,~p}~n", [get_now_time(),Name, return_resource, State]),
  NewState =
